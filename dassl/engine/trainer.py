...
class SimpleNet(nn.Module):
    ...
    def __init__(self, cfg, model_cfg, num_classes, **kwargs):
        ...
        if num_classes > 0:
            ...
            self.classifier2 = nn.Linear(fdim, num_classes)
    ...
    def forward(self, x, return_feature=False):
        #TODO:
        [x_adr, x_ce], [branch_out, b2_out], fm = self.backbone(x)
        ...
        y_ce = self.classifier(x_ce)
        y_adr = self.classifier(x_adr)
        

        if return_feature:
            return y_ce, y_adr, branch_out, fm

        return y_ce, y_adr, branch_out
...
class SimpleTrainer(TrainerBase):
    ...
    def build_model(self):
        ...
        if cfg.MODEL.INIT_WEIGHTS:
            load_pretrained_weights(self.model.backbone, cfg.MODEL.INIT_WEIGHTS)
