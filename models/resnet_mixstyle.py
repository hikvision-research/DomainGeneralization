...
def init_pretrained_weights(model, model_url):
    ...
    try:
        pretrained_path = '/data1/pretrained/resnet18.pth'
        load_params = torch.load(pretrained_path)
        model.load_state_dict(load_params, strict=False)
        print('Load Pre-trained model from {}'.format(pretrained_path))
...
def resnet18_baseline(pretrained=True, **kwargs):
    model = ResNet(
        block=BasicBlock,
        layers=[2, 2, 2, 2],
        # mixstyle_layers=[],
